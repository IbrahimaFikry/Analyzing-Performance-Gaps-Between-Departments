...
# Monthly Salary distribution plot
p_M_Salary <- ggplot(df_employee, aes(x = Monthly_Salary)) +
  geom_histogram(
    binwidth = 500, # Bins of $500
    fill = "#151931", # Dark blue fill
  ...
  labs(
    ...
    subtitle = paste("Median monthly salary: $", format(median(df_employee$Monthly_Salary), big.mark=","),
                     " | n =", format(nrow(df_employee), big.mark=",")),
  ...
  theme(
    ...
    axis.text.x = element_text(angle = 45, hjust = 1, size = 10), # Angle x-axis labels for readability
    axis.title.y = element_text(margin = margin(r = 10)),
    panel.grid.major.x = element_blank(), # Remove major x grid lines
    plot.caption = element_text(hjust = 0, margin = margin(t = 10)),
    panel.grid.major = element_blank(), # Remove all major grid lines for cleaner look
    panel.grid.minor = element_blank() # Remove all minor grid lines
  ) +
  scale_x_continuous(
    labels = dollar_format(),  # Format as $10,000
    breaks = seq(0, max(df_employee$Monthly_Salary), by = 500)  # Custom breaks for better readability
  ) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.1))) + # Expand y-axis to accommodate labels
  # Add vertical line for median salary
  geom_vline(
    xintercept = median(df_employee$Monthly_Salary),
    color = "#faca80", # Accent color for median line
    linetype = "dashed",
    linewidth = 1,
    alpha = 0.8 # Slightly transparent
  ) +
  # Annotate median line directly on the plot
  annotate(
    "text",
    x = median(df_employee$Monthly_Salary) * 1.1, # Position annotation slightly to the right of the line
    y = max(ggplot_build(ggplot(df_employee, aes(x = Monthly_Salary)) + geom_histogram(binwidth = 500))$data[[1]]$count) * 0.9, # Dynamically adjust y-position
...
Resign_plot <- ggplot(Resign_data, aes(x = Resigned, y = Count, fill = Resigned)) +
  ...
  labs(
    ...
    subtitle = paste(format(Resign_data$Count[Resign_data$Resigned == "Resigned"], big.mark = ","),
                    " employees", sep = ""),
    x = NULL, # Remove x-axis label
    y = NULL, # Remove y-axis label
...
age_plot <- ggplot(df_employee, aes(x = Age)) +
  ...
  scale_y_continuous(
    ...
    labels = comma_format() # Formats numbers with commas (e.g., 10,000)
  ) +
  # Add titles and captions with markdown support
  labs(
    title = "<span style='font-size:18pt'>Employee Age distribution shows <span style='color:#151931'><br>key Workforce demographics</span></span>",
    subtitle = paste(
      "Precise 1-year bins |",
...
plot_salary <- ggplot(df_employee, aes(x = Resigned, y = Monthly_Salary, fill = Resigned)) +
  ...
  labs(
    title = "<span style='font-size:18pt'>Salary distribution doesn't show <span style='color:#151931'> <br>clear differences</span> by Resignation Status</span>",
...
plot_satisfaction <- ggplot(df_employee,
  ...
  # Titles and labels with markdown
  labs(
    title = "<span style='font-size:18pt'>Satisfaction scores show <br> <span style='color:#151931'>No apparent difference Gap</span> between Groups</span>",
...
# Calculate resignation rates by Remote_Work_Frequency
...
# Create plot for remote work resignation rates
remote_work_plot <- ggplot(resignation_data_remote, # Use the renamed data variable
                          aes(x = fct_reorder(Remote_Work_Frequency, -Resignation_Rate),
                              y = Resignation_Rate,
  ...
  scale_fill_gradient(low = "#faca80", high = "#151931", guide = "none") +
  labs(
    title = "<span style='font-size:18pt'>Fully office-based staff show<span style='color:#151931'><br>highest attrition</span> (10.0%)</span>", # Updated title to reflect the finding
    subtitle = "Resignation rates decline as remote work frequency increases",
    x = "Remote Work Frequency (%)", # Clearer x-axis label
    y = "Resignation Rate (%)",
...
# Create plot for performance score and resignation rates
performance_plot_resignation <- ggplot(performance_data_resignation, # Use the renamed data variable
                         aes(x = fct_reorder(Performance_Score, -Resignation_Rate),
                             y = Resignation_Rate,
                             fill = Resignation_Rate)) +
  geom_col(width = 0.6) +
  geom_text(aes(label = sprintf("%.2f%%", Resignation_Rate)),
  ...
  scale_fill_gradient(low = "#faca80", high = "#151931", guide = "none") +
  labs(
    title = "<span style='font-size:18pt'>Lower performers show <span style='color:#151931'>higher attrition</span> (10.2%)</span>", # Updated title
    subtitle = "Resignation rates decrease as performance scores improve",
    x = "Performance Score (1-5)", # Clearer x-axis label
    y = "Resignation Rate (%)",
...
tenure_plot <- ggplot(resignation_by_tenure,
                     aes(x = fct_reorder(tenure_bin, resignation_rate), # Order by resignation rate
                         y = resignation_rate,
                         fill = resignation_rate)) +
  geom_col(width = 0.8) +
  geom_text(
  ...
  labs(
    title = "<span style='font-size:18pt'>Highest attrition (15.9%) among oldest employees<br> (0-1 year)</span>", 
    subtitle = "Resignation rates decline with tenure, then rise again after 10+ years",
    x = "Years at Company", 
    y = "Resignation Rate (%)",
...
performance_plot_department <- ggplot(performance_data_department, # Use the renamed data variable
  ...
  labs(
    ...
    subtitle = "Marketing trails by -0.38 points from company average",
    x = NULL, # Remove x-axis label
    y = NULL, # Remove y-axis label
...
edu_plot_performance <- ggplot(edu_performance, # Use the renamed data variable
  ...
  # Titles and labels with markdown
  labs(
    title = "<span style='font-size:18pt'>PhD holders outperform by +0.03 points</span>",
    subtitle = "High school graduates trail company average by -0.02 points",
    x = NULL, # Remove x-axis label
    y = NULL, # Remove y-axis label
    caption = paste("Performance scale: 1-5 | Company average:",
...
overtime_plot <- ggplot(overtime_data_performance, # Use the renamed data variable
                       aes(x = fct_reorder(Overtime_Bin, avg_score), # Reorder by average score
                           y = avg_score,
                           fill = highlight)) +
  geom_col(width = 0.7) +
  # Reference line for overall company average performance
  ...
  # Titles and labels with markdown
  labs(
    title = "<span style='font-size:18pt'>Moderate overtime (10-14h)<br> correlates with peak performance (3.01)</span>",
    subtitle = "Employees working 25+ hours overtime show 0.02 lower scores than average",
